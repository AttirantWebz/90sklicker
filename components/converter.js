...
export default async function generatePDF(Qdata) {
  ...
  const existingPdfBytes = await fetch("/90sQT.pdf").then((res) =>
    res.arrayBuffer()
  );
  ...
  //get font
  const fontBytes = await fetch("/beb.otf").then((res) => res.arrayBuffer());

  const tnrfonts = await fetch("/tnr.ttf").then((res) => res.arrayBuffer());
