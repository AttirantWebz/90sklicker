...
export default async function xandy(Page, Qdata, fnt, wd, ht, tfnt) {
  //qutation number
  console.log("get price in str:", itos(Qdata.price))
  let n = cptls(`${itos(Qdata.price)} only`)
  const dd = Qdata.date.split("-")
  let date = new Date(Qdata.date)
  let txt = `${mon[date.getMonth() - 1]} ${Qdata.date[2]} / ${Qdata.location}`;
